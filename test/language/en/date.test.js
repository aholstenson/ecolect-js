		...
		*/
		...
		describe('Full dates', function() {
			...
			it('end of week - Sunday start', function() {
				return date('end of week', { now: new Date(2010, 0, 1), weekStart: 7 })
				.then(v =>
					expect(v).to.deep.equal({ year: 2010, month: 0, day: 2 })
				);
			});
			...
			it('end of week', function() {
				return date('end of week', { now: new Date(2010, 0, 1) })
				.then(v =>
					expect(v).to.deep.equal({ year: 2010, month: 0, day: 3 })
				);
			});
			...
			it('start of week', function() {
				return date('start of week', { now: new Date(2010, 0, 1) })
				.then(v =>
					expect(v).to.deep.equal({ year: 2009, month: 11, day: 28 })
				);
			});
